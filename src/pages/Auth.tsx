...
export default function Auth() {
  ...
  return (
    <div className="min-h-screen grid lg:grid-cols-2">
      ...
      {/* Form Side */}
      <div className="flex items-center justify-center p-6 sm:p-8 bg-slate-900 overflow-y-auto">
        <motion.div 
        ...
        >
          <div className="text-center space-y-4 md:space-y-6">
            ...
            <div className="flex justify-center gap-3 md:gap-4">
              <button 
                ...
                className={`text-[9px] md:text-[10px] font-black uppercase tracking-widest px-4 py-2.5 rounded-xl transition-all ${isLogin ? 'bg-cyan-500 text-white shadow-xl shadow-cyan-500/20 px-6' : 'text-white/20 hover:text-white'}`}
              >
                Sign In
              </button>
              <button 
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <AnimatePresence mode="wait">
              {!isLogin && (
                <motion.div
                  ...
                  className="space-y-4"
                >
                  <label className="text-[9px] md:text-[10px] font-black text-white/20 uppercase tracking-[0.3em] ml-2">Username</label>
                  <div className="relative group">
                    ...
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl py-4 md:py-5 pl-12 md:pl-14 pr-6 md:pr-8 focus:outline-none focus:border-cyan-500 transition-all font-medium text-white placeholder:text-white/10 text-sm md:text-base"
                    />
                  </div>
                </motion.div>
              )}
            ...
            <div className="space-y-3 md:space-y-4">
              <label className="text-[9px] md:text-[10px] font-black text-white/20 uppercase tracking-[0.3em] ml-2">Email Address</label>
              <div className="relative group">
                <Mail size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-cyan-400 transition-colors md:w-[18px] md:h-[18px]" />
                <input 
                  type="email"
                  required
                  value={email}
            ...
            <div className="space-y-3 md:space-y-4">
              <label className="text-[9px] md:text-[10px] font-black text-white/20 uppercase tracking-[0.3em] ml-2">Secure Cipher</label>
              <div className="relative group">
                <Lock size={16} className="absolute left-5 top-1/2 -translate-y-1/2 text-white/20 group-focus-within:text-cyan-400 transition-colors md:w-[18px] md:h-[18px]" />
                <input 
                  type="password"
                  required
                  value={password}
